...
Left Section (auth-left-section scrollbar-hide-default): Render a top logo link <Link className="auth-logo" href="/"> with Next.js <Image alt="TradeSignal logo" className="h-8 w-auto" height={32} src="/assets/icons/logo.svg" width={140}/> . Below the logo, render <div className="pb-6 lg:pb-8 flex-1">{children}</div> to host dynamic form content.
